; ByteRub Hayek GUI Wallet Installer for Windows
; Copyright (c) 2014-2018, The Monero Project
; See LICENSE

[Setup]
AppName=ByteRub GUI Wallet
; For InnoSetup this is the property that uniquely identifies the application as such
; Thus it's important to keep this stable over releases
; With a different "AppName" InnoSetup would treat a mere update as a completely new application and thus mess up

AppVersion=0.11.1.0
DefaultDirName={pf}\ByteRub GUI Wallet
DefaultGroupName=ByteRub GUI Wallet
UninstallDisplayIcon={app}\byterub-wallet-gui.exe
...
; ByteRub GUI wallet
Source: "bin\byterub-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
...
Source: "byterub-wallet-gui.log"; DestDir: "{app}"; Flags: onlyifdoesntexist; Permissions: users-modify

; Monero CLI wallet
Source: "bin\byterub-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion

; Monero wallet RPC interface implementation
Source: "bin\byterub-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Monero daemon
Source: "bin\byterubd.exe"; DestDir: "{app}"; Flags: ignoreversion

; Monero daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "byterub-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Monero blockchain utilities
Source: "bin\byterub-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\byterub-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion

; was present in 0.10.3.1, not present anymore in 0.11.1.0
; Source: "bin\byterub-utils-deserialize.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\byterub-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "byterub-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Monero daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\byterubd.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\byterub-daemon.bat'), s, false); 
  end;
end;
...
Name: "{group}\GUI Wallet"; Filename: "{app}\byterub-wallet-gui.exe"
...
Name: "{group}\Utilities\Monero Daemon"; Filename: "{app}\byterubd.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\byterub-wallet-cli.exe"; WorkingDir: "{userdocs}\Monero\wallets"
...
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: "{app}\byterub-wallet-gui.log"
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\byterub-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM byterubd.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{userdesktop}\GUI Wallet"; Filename: "{app}\byterub-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\byterub-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\byterub-project\byterub-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\byterub-project\byterub-core"; ValueType: string; ValueName: "daemonFlags"; ValueData: {code:DaemonFlags};
